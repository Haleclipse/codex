...
pub(crate) struct ChatComposer {
    ...
    // @cometix: statusline/cxline fields
    #[allow(dead_code)]
    is_zellij: bool,
...
impl ChatComposer {
    ...
    pub fn cursor_pos(&self, _area: Rect) -> Option<(u16, u16)> {
        // @cometix: hide native terminal cursor — shown via REVERSED in textarea
        None
    }
    ...
    fn footer_props(&self) -> FooterProps {
        ...
        FooterProps {
            ...
            status_line_enabled: self.statusline_config.enabled || self.footer.status_line_enabled,
...
impl ChatComposer {
    ...
    pub(crate) fn render_with_mask_and_textarea_right_reserve(
        ...
        // @cometix: transparent border lines instead of the upstream
        // `Block::default().style(style)` background fill
        let textarea_style = ratatui::style::Style::default().dim();
        ...
        if composer_rect.height > 1 {
            buf.set_string(
                composer_rect.x,
                composer_rect.y + composer_rect.height - 1,
                "─".repeat(composer_rect.width as usize),
                textarea_style,
            );
        }
        ...
        if !textarea_rect.is_empty() {
            // @cometix: ❯ prompt with Zellij support
            ...
            let prompt = if self.draft.input_enabled {
                if self.draft.is_bash_mode {
                    ...
                    // @cometix: Zellij renders the effort-tier prompt poorly, so keep the plain glyph.
                    ...
                    // @cometix: keep the fork's arrow glyph.
                    "❯"
